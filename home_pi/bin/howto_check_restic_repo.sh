...
## Erstelle einen Mount-Punkt und mounte das Repository:
  ...
  && export PASSFILE="\${SRC}/restic_secret" \\
  && sudo mkdir -p /tmp/restic \\
  && sudo restic -r \${SRC}/restic-repo -p \$PASSFILE mount /tmp/restic --allow-other &
...
## Prüft, ob alle Hashes im Repo korrekt sind und keine Datenpakete fehlen
  export REPO="/mnt/esata/rsnapshots/restic-repo \\
  && export PASSFILE="/mnt/esata/rsnapshots/restic_secret" \\
